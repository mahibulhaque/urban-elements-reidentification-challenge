...
def part_attention_vit_do_train_with_amp(cfg,
    ...
    best_mAP = 0.0
    best_index = 1
    mAP = 0.0          # initialise so checkpoint block never hits NameError
    epoch_metrics = {} # track per-checkpoint metrics for final summary
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                ...
                epoch_metrics[epoch] = {
                    'mAP': mAP, 'R1': cmc[0], 'R5': cmc[4], 'R10': cmc[9]
                }
    ...
    logger.info("")
    logger.info("=" * 65)
    logger.info("CHECKPOINT VALIDATION SUMMARY  (UAM camera-4 queries)")
    logger.info("=" * 65)
    logger.info("  {:>6}  {:>8}  {:>8}  {:>8}  {:>8}".format(
        "Epoch", "mAP", "Rank-1", "Rank-5", "Rank-10"))
    logger.info("  " + "-" * 55)
    for ep in sorted(epoch_metrics.keys()):
        m = epoch_metrics[ep]
        marker = " <-- BEST" if ep == best_index else ""
        logger.info("  {:>6}  {:>8.2%}  {:>8.2%}  {:>8.2%}  {:>8.2%}{}".format(
            ep, m['mAP'], m['R1'], m['R5'], m['R10'], marker))
    logger.info("=" * 65)
    if epoch_metrics:
        best_ep = max(epoch_metrics, key=lambda e: epoch_metrics[e]['mAP'])
        logger.info("BEST CHECKPOINT: epoch {}  |  mAP {:.2%}  |  Rank-1 {:.2%}".format(
            best_ep, epoch_metrics[best_ep]['mAP'], epoch_metrics[best_ep]['R1']))
        logger.info("Checkpoint path: {}".format(
            os.path.join(log_path, cfg.MODEL.NAME + '_{}.pth'.format(best_ep))))
    logger.info("=" * 65)

    # ── final inference with best checkpoint (checkpoints are kept on disk) ──
    ...
    logger.info('Loaded best weights: {}_{}.pth'.format(cfg.MODEL.NAME, best_index))
    for testname in cfg.DATASETS.TEST:
        if 'ALL' in testname:
            testname = 'DG_' + testname.split('_')[1]
        val_loader, num_query = build_reid_test_loader(cfg, testname)
        do_inference(cfg, eval_model, val_loader, num_query)
    logger.info("All checkpoints preserved in: {}".format(log_path))
