...
def part_attention_vit_huge(img_size=(224, 112), stride_size=14, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, **kwargs):
    """Vanilla ViT-Huge (Google ViT) variant of PAT.

    Loads from `pretrained/vit_huge_patch14_224_in21k.pth` (timm
    `vit_huge_patch14_224.orig_in21k`). Patch size = 14 → input 224x112 gives
    a 16x8 patch grid (matching PAT-L's 16x8 at patch=16, image=256x128).
    """
    model = part_Attention_ViT(
        img_size=img_size, patch_size=14, stride_size=stride_size,
